...
function CartIconLink({ className }) {
  const count = useCartStore((s) => s.items.length);
  return (
    <Link
      href="/cart"
      className={className}
      aria-label={count > 0 ? `Shopping cart, ${count} products` : "Shopping cart"}
    >
      <span className="relative inline-flex items-center justify-center">
        <CartBagIcon className="size-5" />
        {count > 0 && (
          <span className="absolute right-0 top-0 z-10 flex h-5 min-w-5 translate-x-[75%] -translate-y-[75%] items-center justify-center rounded-full bg-lime-500 px-1 text-[10px] font-bold tabular-nums text-gray-900 ring-2 ring-white">
            {count > 99 ? "99+" : count}
          </span>
        )}
      </span>
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-gray-200/80 bg-white/60 backdrop-blur-md">
      <div className="relative mx-auto flex w-full max-w-7xl items-center justify-between gap-4 py-3 lg:py-4">
        ...
        {session?.accessToken || session?.data ? (
            ...
            <Dropdown placement="bottom-end">
              <DropdownTrigger>
                <Button
                  ...
                  className="h-10 w-10 shrink-0 rounded-full border border-gray-200 text-gray-700"
                >
