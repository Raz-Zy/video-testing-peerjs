...
function payloadList(res) {
  if (res?.status === "200 OK" && Array.isArray(res?.payload)) return res.payload;
  return [];
}

export default async function Home() {
  const [bestRes, productsRes, categoriesRes] = await Promise.all([
    getBestSellingProductsAction(),
    getProductsAction(),
    getAllCategoriesAction(),
  ]);

  const bestSellers = payloadList(bestRes);
  const essentialsProducts = payloadList(productsRes);
  const essentialsCategories = payloadList(categoriesRes);

  const heroStrip =
    bestSellers.length >= 3
      ? bestSellers.slice(0, 3)
      : essentialsProducts.slice(0, 3);
